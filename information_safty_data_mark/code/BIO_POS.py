...
# 进行标注的内容 路径
file = r"../divide/divide_data/train.txt"
# 标注写入的文件路经
output_path = r"../divide/data_pos/train.txt"
# 存储 不同标注的词 列表
key_list = []
company_list = []
Radical_list = []
...
# 读取词
with open(r"../data/time.txt", "r", encoding='utf-8') as kf:
    for ik in kf:
        key_list.append(ik.strip())

with open(r"../data/location.txt", "r", encoding='utf-8') as cf:
    for ic in cf:
        company_list.append(ic.strip())
with open(r"../data/object.txt", "r", encoding='utf-8') as rf:
    for ir in rf:
